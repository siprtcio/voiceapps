...
func (rivr *RestaurentIVR) InitRestaurentIVR() {
...
func (rivr RestaurentIVR) CreateWelcomeVoiceBot(gatherSayString string) *Response {
	...
	resp.Text = ""
	resp.Gather = &Gather{
		Action:           "https://demo.siprtc.io/SiprtcApplications/UserIntent",
		Method:           "POST",
		Input:            "speech",
...
func (rivr *RestaurentIVR) GetMainMenuResponse() *Response {
	...
	rivr.state = "IDLE"
	return CreateGatherSayResponse(Text, rivr.actionURL, "1")
}
...
func (rivr *RestaurentIVR) GetPeopleNunberCollectionResponse() *Response {
	...
	return CreateGatherSayResponse(Text, rivr.actionURL, "3")
}
...
func (rivr *RestaurentIVR) ProcessDTMFDigits(digits string) *Response {
	if rivr.state == "IDLE" {
		...
		return CreateGatherSayResponse("Press 1 to book table today, Press 2 to book table tomorrow and Press 3 for custom date.", rivr.actionURL, "1")
		...
		if digits == "1" {
			...
			return CreateGatherSayResponse("You can enter date in D D  M M  Y Y Y Y format. for example 10th May 2021 would be 10 05 2021.", rivr.actionURL, "8")
		}
		rivr.state = "COLLECT_SUBTIME"
		return CreateGatherSayResponse("Press 1 for breakfast, press 2 for lunch, press 3 for dinner.", rivr.actionURL, "1")
		...
		return CreateSayHangup("Your table booked for " + rivr.numberofPeopleJoining + " people, " + rivr.timing + " " + rivr.subtime + ", Thankyou for booking table with brabaque nation.")
	}
	return nil
}
